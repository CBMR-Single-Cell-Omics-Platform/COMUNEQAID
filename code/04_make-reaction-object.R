...
foreach(
  q.rnx = rnx.sheet[["reaction_id"]],
  .combine = "c",
  .packages = c(
    "Seurat", "tximport", "SingleCellExperiment", "future",
  ...
  if (!file.exists(unfiltered.file) &
    ...
    if (!file.exists(mat.files.10x)) {
      stop("Not able to locate quants_mat.gz (gene expression)")
    }
    ...
    if ("hto" %in% lib.tib[["library_type"]]) {
      ...
      if (!file.exists(mat.files.hto)) {
        stop("Not able to locate quants_mat.gz (HTO)")
      }
    }
    ...
    call.droplets.list <- call_droplets(
      counts = counts.all,
      protocol = q.protocol,
      cutoff = q.cutoff
    )
    ...
    if ("hto" %in% lib.tib[["library_type"]]) {
      ...
      seur.full <- HTODemux.mcl(seur.full, q_l = 1, q_h = 0.01, cutoff = q.hto.cutoff)
    ...
    if ("hto" %!in% lib.tib[["library_type"]]) {
      ...
      dbl.dens <- scDblFinder::computeDoubletDensity(sce.full,
        subset.row = top.rna,
