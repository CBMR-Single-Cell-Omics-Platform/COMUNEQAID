...
```{r Functions}
tmp.config <- yaml::read_yaml('/maps/projects/scop/data/SCOP_2021_0142_test/output/2023-08-30_brain-cell-line_human/COMUNEQAID_v2.0/COMUNEQAID-pipeline/2023-08-30_brain-cell-line_human.yml')
# snakemake@config -> tmp.config
# Shared functions
source('code/shared.R')

# Paths
project.path <- file.path(
  tmp.config[['project_path']],
  tmp.config[['scop_id']])
...
suppressMessages({
  omnisheet <- 
    as_tibble(tmp.config[['sample_sheet']]) %>% 
    full_join(as_tibble(tmp.config[['sample2reaction']])) %>% 
    full_join(as_tibble(tmp.config[['reaction_sheet']])) %>% 
    full_join(as_tibble(tmp.config[['reaction2library']])) %>% 
    full_join(as_tibble(tmp.config[['library_sheet']])) %>% 
    full_join(as_tibble(tmp.config[['library2sequencing']])) %>% 
    full_join(as_tibble(tmp.config[['sequencing_sheet']]))

  rnxsheet.rnx2lib <-
    full_join(as_tibble(tmp.config[['reaction_sheet']]),
              as_tibble(tmp.config[['reaction2library']]))
  
  seq.sheet <-
    as_tibble(tmp.config[['sequencing_sheet']])
  
  rnx.sheet <-
    as_tibble(tmp.config[['reaction_sheet']])
  
  samplesheet.sample2rnx <- full_join(as_tibble(tmp.config[['sample_sheet']]),
            as_tibble(tmp.config[['sample2reaction']]))
})
