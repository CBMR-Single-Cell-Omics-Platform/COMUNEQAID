...
call_droplets <- function(counts, protocol, cutoff) {
  ...
  if (is.numeric(cutoff)) {
    ...
    bc.annot[["Knee.manual"]] <- NA
    bc.annot[["Inflection.manual"]] <- NA
    bc.annot[["Rank_cutoff"]] <- max(bc.df[["Rank"]][bc.df[["nUMI_RNA"]] > bc.annot[["Manual.thresh"]]])

    valid.bcs <- bc.df[["Barcode"]][bc.df[["nUMI_RNA"]] > bc.annot[["Manual.thresh"]]]
    cat("#\t..\t\t", protocol, ":\t\t", length(valid.bcs), "\n",
      sep = ""
    )
  }
...
HTODemux.mcl <- function(object, assay = "HTO", q_l = 1, q_h = 0.001, cutoff = FALSE, seed = 42) {
...
make_summary_table <- function() {
  ...
  for (rnx.i in rnx.sheet[["reaction_id"]]) {
    ...
    demultiplex.stats <- read_csv(file.path(
      demult.stats.path,
    ...
    if ("hto" %in% lib.tib[["library_type"]]) {
      ...
      # CALLED SINGLETS (INTRA-HTO)
      cells.singl.intra <- rownames(meta.data[meta.data[["HTO_globalClass"]] == "Singlet" & meta.data[["RNA_recoveredDoubletBool"]] == F & meta.data[["RNA_doubletNeighborBool"]] == F, ])
  ...
  stat.df.10x %>%
    ...
    add_header_above(
      c(
        "Summary stats (RNA expression)" = 3,
        " " = 21
      ),
      align = "center",
      color = "#dedede",
  ...
  if ("hto" %in% lib.tib[["library_type"]]) {
    ...
    stat.df.hto %>%
      ...
      add_header_above(
        c(
          "Summary stats (HTO expression)" = 4,
          " " = 22
        ),
        align = "center",
        color = "#dedede",
...
make_plot_top10bcl <- function(df) {
  ...
  p <- ggplot(df) +
    geom_bar(
      mapping = aes(x = reorder(dual_index, reads), y = reads),
      stat = "identity",
      fill = softPallet(1)
    ) +
...
make_plot_barcodeRanks <- function(df, annot, ref, rnx.name, rnx.type) {
  ...
  if (annot[["Manual"]] == T) {
    ...
    inflection.new <- annot[["Manual.thresh"]]

    p1 <- p1 +
      geom_hline(aes(yintercept = inflection.new, linetype = "Manual threshhold"), color = my.cols[["Highlight"]], size = 1) +
      geom_hline(aes(yintercept = knee, linetype = "Knee (old)"), color = my.cols[["Lowlight"]], size = 1) +
      scale_linetype_manual(
        name = "",
        limits = c("Reference", "Manual threshhold", "Knee (old)", "Optimal plateau drop"),
        values = c(3, 2, 2, 5),
    ...
    p2 <- p2 +
      geom_vline(aes(xintercept = inflection.new, linetype = "Manual threshhold"), size = 1, color = my.cols[["Highlight"]]) +
      geom_vline(aes(xintercept = knee, linetype = "Knee (old)"), size = 1, color = my.cols[["Lowlight"]]) +
      scale_linetype_manual(
        name = "",
        limits = c("Manual threshhold", "Knee (old)"),
        values = c(2, 2),
        guide = guide_legend(override.aes = list(color = c(my.cols[["Highlight"]], my.cols[["Lowlight"]])))
      ) +
      ggtitle(rnx.name, subtitle = paste(annot$Rank_cutoff, rnx.type, "\t|\tmanual threshold:", annot[["Manual.thresh"]]))
  }
...
make_plotly_barcodeRanks <- function(df, annot, ref, rnx.name, rnx.type) {
  knee <- annot[["Knee"]]
  inflection <- annot[["Inflection"]]
  ...
  p <- ggplotly(p, tooltip = c("x"))

  return(p)
}

make_plot_htoThresh <- function(hto.sample.calling.metadata, hto.cutoff.metadata, q.rnx) {
...
make_plotly_htoThresh <- function(hto.sample.calling.metadata, hto.cutoff.metadata, q.rnx) {
  hto.cutoff.metadata[["hto"]] <- hto.cutoff.metadata[["hto_name"]]

  breaks <- pretty(range(hto.sample.calling.metadata[["expression"]]),
    n = nclass.scott(hto.sample.calling.metadata[["expression"]]), min.n = 1
  )

  p <- ggplot(hto.sample.calling.metadata, aes(x = expression, fill = above_cutoff)) +
    geom_histogram(bins = length(breaks) * 3) +
    # facet_wrap(~sample_id, scales = 'free',ncol = 3) +
    facet_wrap(~sample_id, ncol = 3) +
    xlab("UMI counts (CLR-norm)") +
    ylab("Cell barcode frequency") +
...
make_plot_htoVlnGlobal <- function(meta.data, rnx.name) {
  ...
  p <- ggplot(
    ...
    scale_fill_manual(
      values = tmp.color.palette,
      limits = levels(meta.data[["HTO_globalClass"]])
    ) +
    scale_y_log10(labels = exponent_format()) +
    annotation_logticks(sides = "l") +
    labs(
      x = "HTO classification (Global)",
      y = "UMI counts (HTO)"
    ) +
    theme_minimal(base_size = base.size) +
    theme(
...
make_plot_htoVlnIndividual <- function(meta.data, rnx.name) {
  ...
  p <- ggplot(
    ...
    annotation_logticks(sides = "l") +
    labs(
      x = "HTO classification (Individual)",
      y = "UMI counts (HTO)"
    ) +
    theme_minimal(base_size = base.size) +
    theme(
...
make_plot_htoVlnAll <- function(meta.data, rnx.name) {
  mutate(meta.data,
    doublet_type = factor(
      ifelse(HTO_doubletBool == T, "Inter-HTO doublet",
      ...
      levels = c("Negative", "Singlet", "Inter-HTO doublet", "Intra-HTO doublet (profile)", "Intra-HTO doublet (neighbours)")
    )
  ) -> meta.data

  p <- ggplot(
    meta.data[meta.data$HTO_globalClass != "Negative", ],
    aes(
      x = doublet_type,
      y = nCount_RNA,
      fill = doublet_type
    ...
    geom_jitter(alpha = .25) +
    scale_fill_manual(
      values = c(my.cols[["Singlet"]], my.cols[["Doublet"]], my.cols[["Doublet"]], my.cols[["Doublet"]]),
...
demux_counts <- function(stats_folder,
  ...
  undetermined <- data.frame(
    "reaction_id" = "Undetermined",
...
merge_sheets <- function(..., config = snakemake@config) {
  ...
  reduce(
    .x = config[table_names],
    .f = merge.data.frame
  )
}

process_match <- function(x) {
  num_str <- gsub('"nUMI_RNA: |"', "", x) # Remove non-numeric characters including quotes
  num <- as.numeric(num_str) # Convert to numeric
  if (!is.na(num)) {
    transformed_num <- round(10^num) # Calculate 10 raised to the power of the number
    return(sprintf('"nUMI_RNA: %d"', transformed_num)) # Return the formatted string
  } else {
    return(x) # If NA, return the original string
  }
}

transform_numbers <- function(line) {
  if (grepl('"text":\\[', line)) {
    # Use gregexpr and regmatches for more controlled replacement
    matches <- gregexpr('"nUMI_RNA: ([0-9\\.]+)"', line, perl = TRUE)
    all_matches <- regmatches(line, matches)
    if (length(all_matches[[1]]) > 0) {
      # Apply transformation to each match found
      transformed_matches <- lapply(all_matches[[1]], process_match)
      regmatches(line, matches) <- list(transformed_matches)
    }
  }
  return(line)
}
